...
class FuzzyTestCase(Base):
    ...
    # 一个测试用例可含有多个 Block 字段
    block_field = relationship('BlockField', back_populates="fuzzing_case")
    
    # 一个测试用例可含有多个 Byte 字段
    byte_field = relationship('ByteField', back_populates="fuzzing_case")
    # 一个测试用例可含有多个 Bytes 字段
    bytes_field = relationship('BytesField', back_populates="fuzzing_case")
...
class BlockField(Base):
    ...
    fuzzing_case = relationship("FuzzyTestCase", back_populates="block_field")
    
    # Block 字段可含有其它的字段，这是一个比较特殊的点！
...
class ByteField(Base):
    ...
    # 外键是模糊测试用例 id
    fuzzing_case_id = Column(Integer, ForeignKey("fuzzy_test_cases.id"))
    fuzzing_case = relationship("FuzzyTestCase", back_populates="byte_field")
...
class BytesField(Base):
    ...
    # 外键是模糊测试用例 id
    fuzzing_case_id = Column(Integer, ForeignKey("fuzzy_test_cases.id"))
    fuzzing_case = relationship("FuzzyTestCase", back_populates="bytes_field")
